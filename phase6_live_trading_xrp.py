...
# Final 2 minutes: compare spot XRP to market strike; within $3 blocks new entries (flat) or triggers exit (position)
FINAL_PHASE_XRP_RULE_SECONDS = 120
XRP_TARGET_PROXIMITY_DOLLARS = 0.001
XRP_SPOT_POLL_INTERVAL_SECONDS = 5
...
CSV_LOG_PATH = Path("csv_trading_logs/xrp_trades_log.csv")
CSV_HEADERS = [
...
DAILY_NET_LOG_PATH = Path("csv_trading_logs/xrp_daily_net_changes.csv")
...
SESSIONS_DIR = Path("trading_sessions_live_xrp")
...
def log_completed_daily_net_changes():
    """Log completed midnight-to-midnight daily balance changes for XRP sessions."""
    try:
        ensure_daily_net_csv_exists()
        ...
        for row in rows_to_write:
            print(
                f"XRP daily net-change logged for {row[0]}: "
...
def xrp_strike_target_usd(market_info: dict) -> float | None:
    """Single reference USD level from Kalshi market strikes (for proximity vs spot XRP)."""
...
def fetch_xrp_spot_usd_sync() -> float | None:
    """Spot XRP/USD (public Coinbase quote; used only in final minutes of the round)."""
    try:
...
async def run_live_trading(client: KalshiClient, market_ticker: str, initial_balance: float, session_number: int):
    ...
    xrp_target_usd = xrp_strike_target_usd(market_info)

    print("\n" + "=" * 80)
    print("PHASE 6: LIVE TRADING SYSTEM (XRP)")
    ...
    print(f"Exit Rule: Near strike in final {FINAL_PHASE_XRP_RULE_SECONDS // 60}m (spot vs strike) -> sell; "
    ...
    if xrp_target_usd is not None:
        print(
            f"Final {FINAL_PHASE_XRP_RULE_SECONDS // 60}m XRP rule: spot within ${XRP_TARGET_PROXIMITY_DOLLARS:.2f} "
            f"of strike ${xrp_target_usd:,.4f} -> no new entries if flat; sell if holding"
        )
    else:
        print("Final-2m XRP proximity rule: disabled (no floor/cap strike on market)")
    ...
    last_xrp_spot_usd = None
    last_xrp_spot_fetch_mono = 0.0
        ...
        try:
            ...
            async with websockets.connect(
                ...
                while True:
                    ...
                    in_final_two_min = 0 < seconds_remaining <= FINAL_PHASE_XRP_RULE_SECONDS

                    if in_final_two_min and xrp_target_usd is not None:
                        mono = time.monotonic()
                        if mono - last_xrp_spot_fetch_mono >= XRP_SPOT_POLL_INTERVAL_SECONDS:
                            last_xrp_spot_fetch_mono = mono
                            fetched = await asyncio.to_thread(fetch_xrp_spot_usd_sync)
                            if fetched is not None:
                                last_xrp_spot_usd = fetched

                    near_target_band = (
                        in_final_two_min
                        and xrp_target_usd is not None
                        and last_xrp_spot_usd is not None
                        and abs(last_xrp_spot_usd - xrp_target_usd) <= XRP_TARGET_PROXIMITY_DOLLARS
                    )
                    ...
                    # Position management: final-2m ETH near strike -> exit; else stop at EXIT_TRIGGER; else hold.
                        ...
                        if (
                            ...
                            if sell_quantity <= 0:
                                ...
                                try:
                                    print(
                                        f"\n[{now}] NEAR-TARGET EXIT: spot XRP ${last_xrp_spot_usd:,.4f} "
                                        f"within ${XRP_TARGET_PROXIMITY_DOLLARS:.2f} of strike ${xrp_target_usd:,.4f} "
                                        f"(final {FINAL_PHASE_XRP_RULE_SECONDS // 60}m)"
                                    )
...
async def main_loop():
    print("Initializing Kalshi client (XRP bot)...")
